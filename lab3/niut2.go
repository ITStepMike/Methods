...
func second(x float64, a []float64) float64 {
	Pn := a[0]
	for i := 1; i < len(X); i++ {
		var M float64 = 1
		for m := 0; m < i; m++ {
...
func main() {
	...
	a := make([]float64, 0)
	for i := 0; i < n; i++ {
		a = append(a, DY[i][n-1-i]/(float64(fact(i))*(math.Pow(h, float64(i)))))
	}

	YY := make([]float64, 0)

	for i := 0; i < len(XX); i++ {
		YY = append(YY, second(XX[i], a))
	}

	fmt.Println("Друга інтерполяційна формула Ньютона: ", YY)
